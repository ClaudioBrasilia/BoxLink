...
import {
  Heart, Bluetooth, Loader2, AlertCircle, X,
...
const DEVICE_TIPS = [
  { name: 'Polar / Garmin / Wahoo', tip: 'Umedeça os sensores da cinta antes de iniciar a busca.' },
  { name: 'Samsung / WearOS',       tip: 'Certifique-se de que o relógio não está conectado a outro app.' },
  { name: 'Apple Watch',            tip: 'Abra o app Heart Graph ou similar que transmita via Bluetooth.' },
  { name: 'Relógios Genéricos',     tip: 'Se aparecer como "Watch", confirme pelo endereço (ex: 41:42).' },
];
...
function NativeVersion({ userId, className }: Props) {
  // 🔧 FIX: usar os nomes corretos que o hook useBluetooth realmente exporta
  const {
    devices,
    status,
    error,          // era "errorMessage" — não existe no hook
    scan,           // era "startScanning" — não existe no hook
    connect,        // era "connectDevice" — não existe no hook
    disconnect,     // era "disconnectDevice" — não existe no hook
    heartRate,      // dado real de BPM do hook
    connectedDevice,
  } = useBluetooth();
  ...
  // 🔧 FIX: devices não tem campo "status" — usar connectedDevice do hook
  const availableDevices = isScanning ? devices.filter(d => d.id !== connectedDevice?.id) : [];
  ...
  return (
    <div className={cn('flex flex-col gap-4 p-4 bg-white/5 border border-white/10 rounded-3xl', className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={cn('p-2.5 rounded-2xl', isConnected ? 'bg-primary/20' : 'bg-white/5')}>
            <Watch className={cn('w-5 h-5', isConnected ? 'text-primary animate-pulse' : 'text-white/20')} />
          </div>
          <div>
        ...
        {isConnected && bpm ? (
          <motion.div key="stats" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center">
                <span className={cn('text-[10px] font-black uppercase tracking-widest', zone?.color)}>{zone?.label}</span>
                <span className="text-white/20 text-[9px] font-black">{connectedDevice?.name}</span>
              </div>
              <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                <motion.div className={cn('h-full', zone?.bar)} initial={{ width: 0 }} animate={{ width: `${Math.min(100, (bpm / 190) * 100)}%` }} />
              </div>
      ...
      {isConnected ? (
        ...
        <button
          onClick={() => !isScanning && !isConnecting && scan()}
          disabled={isScanning || isConnecting}
          className={cn(
            'flex items-center justify-center gap-2 w-full py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(202,253,0,0.2)] disabled:opacity-60 disabled:scale-100',
            isScanning || isConnecting ? 'bg-white/10 text-white/40' : 'bg-primary text-black hover:scale-[1.02]',
          )}
        >
          {isScanning || isConnecting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Heart className="w-4 h-4" />}
          {isScanning ? 'Buscando dispositivos...' : isConnecting ? 'Conectando...' : 'Iniciar Leitura de FC'}
        </button>
      )}
