...
async function getInactivitySettings(): Promise<InactivitySettings> {
  ...
  cachedSettings = data?.inactivity || { enabled: false, minWorkoutsPerWeek: 3, excludeSunday: true };
  cacheExpiry = Date.now() + 5 * 60 * 1000;
  return cachedSettings!;
}

export function useInactivity(checkins: { date: string }[]): InactivityState {
  const [state, setState] = useState<InactivityState>({ checkinsInWindow: 0, requiredWorkouts: 3, missingWorkouts: 0, fadePercent: 0, showSleeping: false });
