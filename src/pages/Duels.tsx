    ...
    try {
      if (allSubmitted) {
        ...
        if (winnerId) {
          ...
          // Notifica ambos sobre o resultado
          const winnerName = duel.challenger_id === winnerId ? duel.challengerName : duel.opponentName;
          const loserName = duel.challenger_id === winnerId ? duel.opponentName : duel.challengerName;
          await createNotification(
            winnerId,
            'duel_result',
            '🏆 Você venceu o duelo!',
        ...
        // Notifica o oponente que ainda não enviou
        const opponentId = user.id === duel.challenger_id ? duel.opponent_id : duel.challenger_id;
        const opponentName = user.id === duel.challenger_id ? duel.opponentName : duel.challengerName;
        await createNotification(
          opponentId,
          'duel_result',
          '⏳ Seu oponente enviou o resultado!',
          `${user.name} já enviou o resultado do duelo de ${duel.type}. Envie o seu para revelar o vencedor!`,
          { duel_id: duel.id }
        );
