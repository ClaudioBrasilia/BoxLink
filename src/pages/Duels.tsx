...
function mapDuelFromDb(r: any): DuelData {
  return {
    ...
    betCoinAmount: r.bet_coins ?? r.bet_coin_amount ?? null,
...
export default function Duels() {
  ...
  const [betCoinAmount, setBetCoinAmount] = useState(50);
  ...
  const handleCreateDuel = async () => {
    ...
    if (betMode) {
      ...
      for (const oppId of selectedOpponents) {
        ...
        if (!opp) continue;
        if (betType === 'xp' || betType === 'both') {
          if ((opp.xp || 0) < betXpAmount) {
            alert(`${opp.name} não tem XP suficiente para a aposta.`); return;
          }
        }
        if (betType === 'coins' || betType === 'both') {
          if ((opp.coins || 0) < betCoinAmount) {
            alert(`${opp.name} não tem moedas suficientes para a aposta.`); return;
          }
        }
      }
    }
  ...
  const handleAccept = async (duelId: string) => {
    ...
    if (duel.betMode) {
      if (duel.betXpAmount && (user.xp || 0) < duel.betXpAmount) {
        alert(`XP insuficiente! Você precisa de ${duel.betXpAmount} XP para aceitar.`); return;
      }
      if (duel.betCoinAmount && (user.coins || 0) < duel.betCoinAmount) {
        alert(`Moedas insuficientes! Você precisa de ${duel.betCoinAmount} moedas para aceitar.`); return;
      }
    }
    ...
    try {
      ...
      if (allAccepted && !duel.betReserved && duel.betMode) {
        const xpAmount = duel.betXpAmount || 0;
        const coinAmount = duel.betCoinAmount || 0;
        const allParts = [duel.challengerId, ...(duel.opponentIds || [])];

        for (const pid of allParts) {
          const p = pid === user.id ? user : (users || []).find(u => u.id === pid);
          if (p) {
            const updates_prof: Record<string, any> = {};
            if (xpAmount > 0) updates_prof.xp = Math.max(0, (p.xp || 0) - xpAmount);
            if (coinAmount > 0) updates_prof.coins = Math.max(0, (p.coins || 0) - coinAmount);

            if (Object.keys(updates_prof).length > 0) {
              await supabase.from('profiles').update(updates_prof).eq('id', pid);
              if (pid === user.id) updateUser({ ...user, ...updates_prof });
            }
          }
        }
  ...
  const handleCancel = async (duelId: string) => {
    ...
    setLoading(true);
    try {
      if (duel.betMode && duel.betReserved && !duel.betSettledAt) {
        const xpAmount = duel.betXpAmount || 0;
        const coinAmount = duel.betCoinAmount || 0;
        const allParts = [duel.challengerId, ...(duel.opponentIds || [])];
        ...
        for (const pid of allParts) {
          ...
          if (p) {
            const updates_prof: Record<string, any> = {};
            if (xpAmount > 0) updates_prof.xp = (p.xp || 0) + xpAmount;
            if (coinAmount > 0) updates_prof.coins = (p.coins || 0) + coinAmount;
  ...
  const handleSubmitResult = async (duel: DuelData) => {
    ...
    try {
      if (allSubmitted) {
        ...
        if (duel.betMode && duel.betReserved && !duel.betSettledAt && winnerId) {
          const xpAmount = duel.betXpAmount || 0;
          const coinAmount = duel.betCoinAmount || 0;
          const losers = allParts.filter(id => id !== winnerId);

          const totalXpWinnings = xpAmount * losers.length;
          const totalCoinWinnings = coinAmount * losers.length;
          ...
          for (const pid of allParts) {
            ...
            if (p) {
              const updates_prof: Record<string, any> = {};
              if (xpAmount > 0) {
                updates_prof.xp = (p.xp || 0) + xpAmount;
                if (pid === winnerId) updates_prof.xp += totalXpWinnings;
              }
              if (coinAmount > 0) {
                updates_prof.coins = (p.coins || 0) + coinAmount;
                if (pid === winnerId) updates_prof.coins += totalCoinWinnings;
              }
  ...
  const handleRematch = async (duel: DuelData) => {
    ...
    setLoading(true);
    try {
      const allOpps = [duel.challengerId, ...duel.opponentIds].filter(id => id !== user.id);
      
      // Reset state for new challenge
      setSelectedOpponents(allOpps);
      setWodId(duel.wodId);
      setCategory(duel.category);
      setBetMode(duel.betMode);
      setBetType(duel.betType || 'xp');
      setBetXpAmount(duel.betXpAmount || 100);
      setBetCoinAmount(duel.betCoinAmount || 50);
      setShowCreate(true);
      
      if (confirm('Deseja iniciar uma VINGANÇA (Rematch) com os mesmos atletas e aposta?')) {
        const newDuelId = `duel_${Date.now()}`;
        const results: Record<string, null> = {};
        [user.id, ...allOpps].forEach(id => { results[id] = null; });
        ...
        await createDuelInDb(newDuel);
        setDuels(prev => [newDuel, ...prev]);

        for (const oppId of allOpps) {
          await createNotification(
            oppId,
            'duel_challenge',
            '⚔️ VINGANÇA!',
            `${user.name} quer uma REVANCHE de ${duel.wodName}! Aceita o desafio?`,
            { duel_id: newDuel.id, challenger_name: user.name }
          );
        }

        alert('Desafio de REVANCHE enviado!');
        setShowCreate(false);
        setActiveTab('pending');
      }
    } catch (e: any) {
      alert(e.message || 'Erro ao criar revanche.');
    } finally {
      setLoading(false);
    }
  };
              ...
              <div key={duel.id} className="bg-surface-container-low rounded-[2.5rem] border border-outline-variant/10 p-6 relative overflow-hidden">
                    ...
                    return (
                      <div key={pid} className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col items-center gap-1">
                          <div className={cn(
                            'w-10 h-10 rounded-full border-2 bg-surface-container-highest flex items-center justify-center font-headline font-black text-base transition-all overflow-hidden relative',
                            pid === duel.challengerId ? 'border-secondary shadow-[0_0_10px_rgba(255,116,57,0.2)]' : 'border-primary',
                            isWinner && 'border-secondary scale-110 ring-2 ring-secondary/50 z-10'
                          )}>
                            <AvatarPreview 
                            ...
                            {/* Winner badge */}
                            {isWinner && (
                              <div className="absolute -top-1 -right-1 bg-secondary text-background rounded-full p-0.5 shadow-lg">
                                <Trophy className="w-2.5 h-2.5" />
                              </div>
                            )}
                          </div>
                          <span className={cn(
                            'text-[9px] font-bold uppercase italic truncate max-w-[56px]',
                            isWinner ? 'text-secondary font-black' : 'text-on-surface'
                          )}>
                ...
                {duel.status === 'finished' && duel.winnerId && (
                  <div className="flex flex-col gap-3 mb-3">
                    ...
                    {/* Vingança button */}
                    {(isChallenger || isOpponent) && !duel.betCanceledAt && (
                      <button
                        onClick={() => handleRematch(duel)}
                        className="w-full bg-secondary/10 hover:bg-secondary/20 text-secondary border border-secondary/20 py-3 rounded-xl font-headline font-black text-[10px] uppercase italic tracking-widest flex items-center justify-center gap-2 transition-all group"
                      >
                        <Swords className="w-3 h-3 group-hover:rotate-12 transition-transform" /> VINGANÇA (REVANCHE)
                      </button>
                    )}
                  </div>
                )}
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center p-4 bg-background/80 backdrop-blur-sm overflow-y-auto">
            ...
            >
              ...
              <div className="space-y-5">
                ...
                {/* Bet mode */}
                <div className="space-y-3 p-5 rounded-3xl bg-secondary/5 border border-secondary/10 relative overflow-hidden">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest flex items-center gap-2">
                        Modo Aposta <Zap className="w-3 h-3 text-secondary" />
                      </p>
                      <p className="text-[9px] text-on-surface-variant/60 italic">O vencedor leva a aposta de todos</p>
                    </div>
                  ...
                  {betMode && (
                    <div className="space-y-4 pt-2 border-t border-secondary/10">
                      ...
                      <div className="flex bg-surface-container-highest p-1 rounded-xl">
                        {(['xp', 'coins', 'both'] as const).map(type => (
                          <button
                            key={type}
                            onClick={() => setBetType(type)}
                            className={cn(
                              'flex-1 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all',
                              betType === type ? 'bg-secondary text-background shadow-md' : 'text-on-surface-variant'
                            )}
                          >
                            {type === 'xp' ? 'XP' : type === 'coins' ? 'MOEDAS' : 'AMBOS'}
                          </button>
                        ))}
                      </div>
                      ...
                      {(betType === 'xp' || betType === 'both') && (
                        <div className="space-y-2">
                          <label className="text-[9px] text-on-surface-variant font-bold uppercase tracking-widest flex justify-between">
                            <span>Aposta em XP (mín. 50)</span>
                            <span className="text-secondary">{user?.xp || 0} XP</span>
                          </label>
                          <div className="flex items-center gap-2">
                            <button onClick={() => setBetXpAmount(p => Math.max(50, p - 50))} className="w-8 h-8 rounded-lg bg-surface-container-highest font-black text-on-surface flex items-center justify-center hover:bg-secondary/20 transition-colors">−</button>
                            <div className="flex-1 bg-surface-container-highest rounded-xl p-2 text-center font-headline font-black text-on-surface text-sm">{betXpAmount} XP</div>
                            <button onClick={() => setBetXpAmount(p => Math.min(user?.xp || 0, p + 50))} className="w-8 h-8 rounded-lg bg-surface-container-highest font-black text-on-surface flex items-center justify-center hover:bg-secondary/20 transition-colors">+</button>
                          </div>
                        </div>
                      )}
                      ...
                      {(betType === 'coins' || betType === 'both') && (
                        <div className="space-y-2">
                          <label className="text-[9px] text-on-surface-variant font-bold uppercase tracking-widest flex justify-between">
                            <span>Aposta em Moedas (mín. 10)</span>
                            <span className="text-primary">{user?.coins || 0} 🪙</span>
                          </label>
                          <div className="flex items-center gap-2">
                            <button onClick={() => setBetCoinAmount(p => Math.max(10, p - 10))} className="w-8 h-8 rounded-lg bg-surface-container-highest font-black text-on-surface flex items-center justify-center hover:bg-primary/20 transition-colors">−</button>
                            <div className="flex-1 bg-surface-container-highest rounded-xl p-2 text-center font-headline font-black text-on-surface text-sm">{betCoinAmount} 🪙</div>
                            <button onClick={() => setBetCoinAmount(p => Math.min(user?.coins || 0, p + 10))} className="w-8 h-8 rounded-lg bg-surface-container-highest font-black text-on-surface flex items-center justify-center hover:bg-primary/20 transition-colors">+</button>
                          </div>
                        </div>
                      )}

                      {selectedOpponents.length > 0 && (
                        <div className="p-3 bg-background/50 rounded-xl border border-secondary/5">
                          <p className="text-[9px] text-on-surface-variant font-medium text-center leading-relaxed">
                            Aposta Total no Pote:<br/>
                            <span className="font-black text-on-surface italic uppercase">
                              {(betType === 'xp' || betType === 'both') && `${betXpAmount * (selectedOpponents.length + 1)} XP`}
                              {betType === 'both' && ' + '}
                              {(betType === 'coins' || betType === 'both') && `${betCoinAmount * (selectedOpponents.length + 1)} Moedas`}
                            </span>
                          </p>
                        </div>
                      )}
