...
export default function Clans() {
  ...
  // ── Admin: excluir/cancelar a temporada atual de vez (sem guardar histórico) ──
  const handleDeleteCurrentSeason = async () => {
    if (!isAdmin) return;
    if (!boxSettings.current_season?.name && clans.length === 0) {
      alert('Não há temporada para excluir.');
      return;
    }
    const seasonName = boxSettings.current_season?.name || 'atual';
    const warn = clans.length > 0
      ? `Excluir a temporada "${seasonName}"? Os ${clans.length} time(s) ativo(s) serão APAGADOS e NÃO irão para o histórico. Esta ação não pode ser desfeita.`
      : `Excluir a temporada "${seasonName}"? Esta ação não pode ser desfeita.`;
    if (!confirm(warn)) return;

    // Remove os times ativos (e seus vínculos) sem arquivar
    if (clans.length > 0) {
      const ids = clans.map((c) => c.id);
      await supabase.from('clan_memberships').delete().in('clan_id', ids);
      const { data: deleted, error: delError } = await supabase.from('clans').delete().in('id', ids).select('id');
      if (delError) {
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-surface-container-highest rounded-2xl p-4 border border-outline-variant/10">
                  ...
                  {(boxSettings.current_season?.name || clans.length > 0) && (
                    <button
                      onClick={handleDeleteCurrentSeason}
                      className="w-full mt-2 py-3 rounded-xl font-headline font-black text-xs uppercase italic bg-error text-on-error flex items-center justify-center gap-2"
                    >
